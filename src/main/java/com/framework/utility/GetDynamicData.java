...
public class GetDynamicData {
	...
	public void getDynamicValues(LinkedHashMap<String, String> data, ExtentTest extentTest, String sheetName) {
		...
		if(!ExcelColumnNameConstant.NumericOperationOnExtractValue.toString().equals("")) {
	...
	public void extractSingleValue(String value, ExtentTest extentTest, LinkedHashMap<String, String> data) {
		try {
			...
			while (matcher.find()) {
				...
				System.out.println("dynamic value to be replace: "+dynamicValue);
				...
				System.out.println("Database value for dynamic variable: "+dataBaseValue);
//				if(dataBaseValue.contains("-")) {
//					dataBaseValue =calculateOperation(dataBaseValue);
//				}
	...
	public String calculateOperation(String value) {
		//try {
				...
				System.out.println(" value to be calculate: "+value);
				String[] splitValue = value.split("-");
				int finalvalue= Integer.parseInt(splitValue[0])-Integer.parseInt(splitValue[1]);
				//data.put(value, data.get(value).replace(Integer.toString(finalvalue)));
	...
	public void calculateValues(String value, ExtentTest extentTest, LinkedHashMap<String, String> data) {
		try {
			String regex = "\\#[a-z|A-Z|0-9|\\.|\\-|\\_]+\\#";
			Pattern pattern = Pattern.compile(regex);
			Matcher matcher = pattern.matcher(data.get(value));
	...
	public void assertSingleValueStaticData(LinkedHashMap<String, String> data, ExtentTest extentTest) {
		//try {
			String assertText = data.get(ExcelColumnNameConstant.TESTASSERTRESPONSE.toString());
			System.out.println("Text to be assert: "+assertText);
			if(assertText.contains(";")) {
				String[] assertValues = assertText.split(";");
				for(int i=0;i<assertValues.length; i++) {
					if (!assertValues[i].contains("#"))
						System.out.println("Loop number : "+i);
						System.out.println("Strings to be asserted in loop number "+i+"with text"+assertValues[i]);
						assertMethod(assertValues[i], extentTest);
				}
			}
			else {
				assertMethod(assertText, extentTest);
	...
	public void assertMethod(String assertText, ExtentTest extentTest) {
		//try {
			...
			//	String values = matcher.group();
				System.out.println("The value to split for assertion: "+assertText);
				String[] splitValues = assertText.split(",");
				if (splitValues.length < 3) {
					String actual = splitValues[0].toLowerCase();
					System.out.println("Actual String for assertion: "+actual);
					if (!TextUtils.isEmpty(actual)) {
						String expected = splitValues[1].toLowerCase();
						System.out.println("Expected String for assertion: "+expected);
